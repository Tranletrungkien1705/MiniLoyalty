...
public class MemberController(ILoyaltyService svc) : Controller
{
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> Kmbh(int id, int? points, string? note)
    {
        try
        {
            var tx = await svc.AwardKmbhAsync(id, points, note);
            TempData["Success"] = $"Đã tặng {tx.Points:N0} điểm khuyến mại bán hàng (HTV).";
        }
        catch (Exception ex) { TempData["Error"] = ex.Message; }
        return RedirectToAction(nameof(Details), new { id });
    }
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> OpenCard(int id, int? points, string? note)
    {
        try
        {
            var tx = await svc.AwardOpenCardAsync(id, points, note);
            TempData["Success"] = $"Đã tặng {tx.Points:N0} điểm mở thẻ mới.";
        }
        catch (Exception ex) { TempData["Error"] = ex.Message; }
        return RedirectToAction(nameof(Details), new { id });
    }
    ...
    [HttpPost, ValidateAntiForgeryToken]
    public async Task<IActionResult> RecordPromotion(int id, int promotionId, int qty, string? note)
    {
        if (qty <= 0) { TempData["Error"] = "Số lượng phải > 0."; return RedirectToAction(nameof(Details), new { id }); }
        var (ok, msg) = await svc.RecordPromotionUseAsync(id, promotionId, qty, note);
        TempData[ok ? "Success" : "Error"] = msg;
        return RedirectToAction(nameof(Details), new { id });
    }
